...
export default function Home(props) {
	...
	let matrix = state.log.filter(_log => Object.values(_log).some(__log => __log.match(state.filter))).map(_log => {
				...
				<Button
					onClick={() => {
						log.delete(_log.uuid, () => {
							setState(state => ({ ...state, log: state.log.filter(__log => __log.uuid !== _log.uuid) }));
						});
					}}
					content="Deletar"
				/>
	...
	return <>
		<Upper>
			...
			<Input label={"Filtrar:"} value={state.filter} onChange={(v) => setState({...state, filter: v.target.value.replace(/\\|\(|\)|\[|\]/g,'')})}/>
			</div>
		</Upper>
		<Table headers={headers} matrix={matrix} sizinsg={sizing} offset={offset} />
	</>;
}
